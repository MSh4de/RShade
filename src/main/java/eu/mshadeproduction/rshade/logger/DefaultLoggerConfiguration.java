...
public class DefaultLoggerConfiguration {
    ...
    static int DEFAULT_LOG_LEVEL_DEFAULT = DefaultSimpleLogger.LOG_LEVEL_INFO;
    ...
    DefaultOutChoice outputChoice = null;
    ...
    void init() {
        ...
        showLogName = getBooleanProperty(SimpleLogger.SHOW_LOG_NAME_KEY, DefaultLoggerConfiguration.SHOW_LOG_NAME_DEFAULT);
    ...
    static int stringToLevel(String levelStr) {
        if ("trace".equalsIgnoreCase(levelStr)) {
            return DefaultSimpleLogger.LOG_LEVEL_TRACE;
        } else if ("debug".equalsIgnoreCase(levelStr)) {
            return DefaultSimpleLogger.LOG_LEVEL_DEBUG;
        } else if ("info".equalsIgnoreCase(levelStr)) {
            return DefaultSimpleLogger.LOG_LEVEL_INFO;
        } else if ("warn".equalsIgnoreCase(levelStr)) {
            return DefaultSimpleLogger.LOG_LEVEL_WARN;
        } else if ("error".equalsIgnoreCase(levelStr)) {
            return DefaultSimpleLogger.LOG_LEVEL_ERROR;
        } else if ("off".equalsIgnoreCase(levelStr)) {
            return DefaultSimpleLogger.LOG_LEVEL_OFF;
        }
        // assume INFO by default
        return DefaultSimpleLogger.LOG_LEVEL_INFO;
    }

    private static DefaultOutChoice computeOutputChoice(String logFile, boolean cacheOutputStream) {
        if ("System.err".equalsIgnoreCase(logFile))
            if (cacheOutputStream)
                return new DefaultOutChoice(DefaultOutChoice.OutputChoiceType.CACHED_SYS_ERR);
            else
                return new DefaultOutChoice(DefaultOutChoice.OutputChoiceType.SYS_ERR);
        else if ("System.out".equalsIgnoreCase(logFile)) {
            if (cacheOutputStream)
                return new DefaultOutChoice(DefaultOutChoice.OutputChoiceType.CACHED_SYS_OUT);
            else
                return new DefaultOutChoice(DefaultOutChoice.OutputChoiceType.SYS_OUT);
        } else {
            try {
                FileOutputStream fos = new FileOutputStream(logFile);
                PrintStream printStream = new PrintStream(fos);
                return new DefaultOutChoice(printStream);
