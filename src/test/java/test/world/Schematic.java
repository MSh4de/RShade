...
import test.world.nbt.ByteArrayTag;
import test.world.nbt.CompoundTag;
...
public class Schematic {
    public static World toWorld(CompoundTag nbt) {
        ...
        System.out.println(String.format("width=%s, length=%s, height=%s", width, length, height));
        ...
        for (int x = 0; x < width; x++) {
            for (y = 0; y < height; y++) {
                for (z = 0; z < length; z++) {
                    ...
                    if (blockIndex < blocksData.length)
